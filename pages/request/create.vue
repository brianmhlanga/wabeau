                    ...
                    <div class="surface-card p-4 shadow-2 border-round p-fluid">
                        <div class="grid formgrid p-fluid">
                           ...
                           <div v-if="calculateTotal() > remainingBudget"  class="flex align-items-start p-4 bg-yellow-100 border-round border-1 border-yellow-300 budgetwarning">
                               ...
                               <div class="col-6">
                                    <Button @click="createDraft()" label="Create Draft" icon="pi pi-file"  v-bind="{ 'disabled': !approver_id}" :class="{ 'p-button-secondary': !approver_id}"/>
                               </div>
                        </div>
                    </div>
                </div>
        ...
        <Dialog v-model:visible="addLineItem" maximizable modal header="Line Item" position="top" :style="{ width: '75vw' }">
                <div class="grid formgrid p-fluid">
                    ...
                    <div class="field mb-4 col-12 md:col-6"> 
                        ...
                        <DropDown @change="getSuppliers(commodity_code_id)" v-model="commodity_code_id"  :options="selected_commodities" filter optionLabel="code" optionValue="id" placeholder="Select Commodity ID" class="w-full" />
                    </div>
                    <div class="field mb-4 col-12 md:col-6"> 
...
<script setup lang="ts">
    ...
    const createRequest = async () => {
        ...
        let result = await adminStore.createRequest(data)
        if (result.data.success) {
    ...
    const getSuppliers = async (id) => {
        ...
        let buyer = await adminStore.getSelectedSuppliers(id)
    }
