                    ...
                    <div v-if="display_toggle === true" class="text-900 font-medium text-xl mb-3">Requisition Details</div>
                           <div v-if="display_toggle === false" class="field mb-4 col-12 md:col-12"> 
                               ...
                               <DataTable :value="requisitions" showGridlines tableStyle="min-width: 50rem">
                                   ...
                                   <Column field="rating" header="Next Approver">
                                       <template #body="slotProps">
                                           {{slotProps?.data?.approver?.name}} {{slotProps?.data?.approver?.surname}}
                                       </template>
                                   </Column>
                                   ...
                                   <Column header="Actions">
                                       <template #body="slotProps">
                                           ...
                                           <Button v-if="slotProps?.data?.status === 'DRAFT'" @click="navigateTo(`odetail-${slotProps.data?.id}`)" icon="pi pi-pencil" outlined rounded class="mr-2"/>
                                       </template>
                                   </Column>
                               </DataTable>
                           </div>
...
<script setup lang="ts">
   ...
   onMounted( async ()=> {
       ...
       let requests = await adminStore.getRequests(await authStore.user_id).then((data)=> {
