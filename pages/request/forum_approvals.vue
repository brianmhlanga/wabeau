                        ...
                        <div class="grid formgrid p-fluid">
                            <div v-if="display_toggle === false" class="field mb-4 col-12 md:col-12"> 
                                ...
                                <DataTable :value="pending" showGridlines tableStyle="min-width: 50rem">
                                    ...
                                    <!-- <Column field="rating" header="Supplier">
                                        <template #body="slotProps">
                                            {{slotProps?.data?.supplier?.supplier_code}}
                                        </template>
                                    </Column> -->
                                    ...
                                    <Column field="rating" header="Requisition Total">
                                        <template #body="slotProps">
                                            {{slotProps?.data?.total ? formatCurrency(slotProps?.data?.total) : formatCurrency(0)}}
                                        </template>
                                    </Column>
                                    <Column field="rating" header="Approval Progress">
                                        <template #body="slotProps">
                                            <ProgressBar :value="((slotProps?.data?.current_forum_approvers/slotProps?.data?.required_forum_approvers)*100)"> {{ slotProps?.data?.current_forum_approvers }}/{{ slotProps?.data?.required_forum_approvers }} </ProgressBar>
                                        </template>
                                    </Column>
                                    <Column header="Actions">
                                        <template #body="slotProps">
                                            ...
                                            <Button v-if="!checkAlreadyApproved(slotProps?.data?.forum_approvers)" @click="returnRequisition(slotProps.data?.id)"  icon="pi pi-times" outlined rounded class="mr-2"/>
                                            <Button v-if="!checkAlreadyApproved(slotProps?.data?.forum_approvers)" @click="forwardRequisition(slotProps.data?.id)"  icon="pi pi-check" outlined rounded class="mr-2"/>
                                        </template>
                                    </Column>
                                </DataTable>
                            </div>
...
<script setup lang="ts">
    ...
    onMounted( async ()=> {
        ...
        let pending = await adminStore.getForumPendingRequests(id)
    })
    ...
    const checkAlreadyApproved = (value) => {
        let id =user_id.value
        for (const approver of value) {
        if (approver.approver_id === id) {
            return true;
            }
        }
        return false;
        ...
        accept: async () => {
            let my_object = {
              ...
              if (result.data.success) {
                    ...
                    await adminStore.getForumPendingRequests(user_id.value)
